...
(function() {
    ...
    function processTradeLogs(logs, targetId) {
        ...
        Object.entries(groups).forEach(function(e) {
            ...
            if (hasOtherItems) {
                ...
                // 只有目标物品，直接用总金额
                cost = trade.money;
                costEach = Math.round(cost / totalQty);
            }

            trades.push({
                id: tradeId, type: 'trade', typeName: 'Trade', timestamp: trade.timestamp,
                qty: totalQty, costEach: costEach, costTotal: cost, seller: trade.user,
    ...
    function renderResults(purchases, itemName) {
        // 先计算不含其他物品的交易的均价
        var cleanPurchases = purchases.filter(function(p) { return !p.hasOtherItems; });
        var cleanQty = cleanPurchases.reduce(function(s, p) { return s + p.qty; }, 0);
        var cleanCost = cleanPurchases.reduce(function(s, p) { return s + p.costTotal; }, 0);
        var avgPrice = cleanQty > 0 ? Math.round(cleanCost / cleanQty) : 0;

        // 用均价填充含其他物品的交易
        purchases.forEach(function(p) {
            if (p.hasOtherItems) {
                p.costEach = avgPrice;
                p.costTotal = avgPrice * p.qty;
            }
        });
        ...
        purchases.forEach(function(p) {
            ...
            var extraInfo = p.hasOtherItems ? ' <span style="color:#ffc107;">(含其他物品，按当前均价计算)</span>' : '';
