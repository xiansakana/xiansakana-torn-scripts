...
(function() {
    ...
    */
    ...
    let userCash = { company: 0, vault: 0 }; // 缓存用户现金数据
    const t_red = "#d83500";
    const t_green = "#678c00";
    let positionSortAsc = false; // 仓位排序方向，false为降序（默认）
    let cachedTransactions = []; // 缓存交易数据
    ...
    function appendTable() {
        ...
        */
        $("#stockmarketroot").before(`
			...
			<div id="istock-cash-position" style="margin:0 0 10px; text-align:center;">
                <span style="padding:5px; color:#fff; font-weight:bold; background:rgb(89, 105, 133);">&#29616;&#37329;&#20179;&#20301;</span>
                <span id="cash_position" title="&#20844;&#21496;&#29616;&#37329; + &#37329;&#24211;&#29616;&#37329;" style="padding:5px; background:rgb(44, 53, 72); border-radius:2px; color:#fff; font-weight:bold;"></span>
            </div>
            <div id="istock-table" style="margin-bottom:10px">
				<table style="margin: auto; background-color: white;">
					<tr class="istock-head">
						...
						<th class="istock-position mobile-hide" style="cursor:pointer;">仓位% <span id="sort-position">▼</span></th>
						...
						<th class="istock-merge mobile-hide">合并显示</th>
						<th class="istock-exclude mobile-hide">分红投资</th>
					</tr>
				</table>
			</div>
        ...
        */
        ...
        APIDetect("user", "", "stocks,personalstats", function(res) {
            ...
            获取用户现金数据（公司和金库）
            */
            fetchUserMoney().then(function(moneyData) {
                userCash = moneyData;
                console.log("用户现金数据:", userCash);
                
                // 继续处理股票数据
                processStockData(res);
    ...
    function setupCompactLayout() {
        if ($("#istock-compact-style").length) {
            return;
        }
        $("head").append(`
            <style id="istock-compact-style">
                #istock-table { max-width: 100%; overflow: visible; }
                #istock-table table { border-collapse: collapse; }
                #istock-table .mobile-hide,
                #istock-table .mobile-show { display: table-cell !important; }
                @media (max-width: 768px) {
                    #istock-table { overflow-x: auto; -webkit-overflow-scrolling: touch; }
                    #istock-table table { min-width: 820px; }
                    #istock-table th, #istock-table td { white-space: nowrap; }
                }
            </style>
        `);
    }
    ...
    function processStockData(res) {
        ...
        */
        ...
        // 缓存交易数据
        cachedTransactions = sorted_res;

        // 添加仓位排序点击事件
        $("th.istock-position").click(function() {
            positionSortAsc = !positionSortAsc;
            $("#sort-position").text(positionSortAsc ? "▲" : "▼");
            renderTable(sorted_res);
        });
        ...
        */
            ...
            */
            ...
            checkAll($("th.istock-merge"), $(".istock-merge-checkbox"));
            checkAll($("th.istock-exclude"), $(".istock-exclude-checkbox"));
            /*
            显示与隐藏模块
            */
            ...
            */
            ...
            $("tr.istock-content").each(function() {
                ...
                if (ls_object != undefined && ls_object != null) {
                    if (ls_object.merge == 1) {
                        $(this).children(".istock-merge").children().prop("checked", true);
                    }
                    if (ls_object.exclude == 1) {
                        $(this).children(".istock-exclude").children().prop("checked", true);
                    }
                }
            });
    }

    function renderTable(sorted_res) {
        // 清空表格内容（保留表头）
        $("#istock-table table tr.istock-content").remove();

        // 读取localStorage中的合并和分红投资设置
        let mergeSettings = {};
        let excludeSettings = {};
        sorted_res.forEach(function(item) {
            const ls_object = getLocalStorage("ISTOCK", item.transaction_id);
            if (ls_object != undefined && ls_object != null) {
                mergeSettings[item.transaction_id] = ls_object.merge == 1;
                excludeSettings[item.transaction_id] = ls_object.exclude == 1;
            } else {
                mergeSettings[item.transaction_id] = false;
                excludeSettings[item.transaction_id] = false;
            }
        });
        ...
        sorted_res.forEach(function(item) {
            ...
            const trans_id = item.transaction_id;

            if (!stockGroups[stock_id]) {
                stockGroups[stock_id] = { transactions: [], merged_ids: [], total_value: 0 };
            }
            stockGroups[stock_id].transactions.push(item);

            // 如果标记为合并且不是分红投资，则累加价值
            if (mergeSettings[trans_id] && !excludeSettings[trans_id]) {
                stockGroups[stock_id].merged_ids.push(trans_id);
                stockGroups[stock_id].total_value += item.shares * item.bought_price;
            }
        });
        ...
        // 计算投机仓位总股本（用于仓位百分比）
        let speculative_cap = 0;
        displayRows.forEach(function(row) {
            if (!excludeSettings[row.transaction_id]) {
                speculative_cap += row.shares * row.bought_price;
            }
        });
        ...
        displayRows.forEach(function(row) {
            ...
            const trans_id = row.transaction_id;

            if (excludeSettings[trans_id]) {
                row.position_percent = -1; // 标记为分红投资
            } else if (speculative_cap > 0) {
                // 检查是否属于合并组
                if (stockGroups[stock_id] && stockGroups[stock_id].merged_ids.includes(trans_id)) {
                    // 使用合并后的总价值计算仓位
                    row.position_percent = (stockGroups[stock_id].total_value / speculative_cap) * 100;
                } else {
                    // 使用单笔交易的价值计算仓位
                    row.position_percent = (row.shares * row.bought_price / speculative_cap) * 100;
                }
            } else {
                row.position_percent = 0;
            }
        });

        // 按仓位排序（分红投资始终排在最后）
        displayRows.sort(function(a, b) {
            const aIsDividend = excludeSettings[a.transaction_id];
            const bIsDividend = excludeSettings[b.transaction_id];

            // 如果一个是分红投资，一个不是，非分红投资排在前面
            if (aIsDividend && !bIsDividend) {
                return 1; // a是分红，b不是，a排后面
            }
            if (!aIsDividend && bIsDividend) {
                return -1; // a不是分红，b是，a排前面
            }

            // 如果都是分红投资或都不是分红投资，按仓位百分比排序
            if (positionSortAsc) {
                return a.position_percent - b.position_percent;
            } else {
                return b.position_percent - a.position_percent;
            }
        });

        // 渲染表格行
        // 首先找出每个股票的第一个合并交易
        let firstMergedTransaction = {};
        displayRows.forEach(function(value) {
            const stock_id = value.stock_id;
            const trans_id = value.transaction_id;

            if (stockGroups[stock_id] && stockGroups[stock_id].merged_ids.includes(trans_id)) {
                if (!firstMergedTransaction[stock_id]) {
                    firstMergedTransaction[stock_id] = trans_id;
                }
            }
        });

        displayRows.forEach(function(value) {
            const isDividend = excludeSettings[value.transaction_id];
            const stock_id = value.stock_id;
            const trans_id = value.transaction_id;
            ...
            if (isDividend) {
                positionText = "分红";
                positionColor = "#666";
            } else if (value.position_percent >= 0) {
                positionText = value.position_percent.toFixed(2) + "%";

                // 检查是否属于合并组
                if (stockGroups[stock_id] && stockGroups[stock_id].merged_ids.includes(trans_id)) {
                    // 只有第一个合并交易显示仓位%，其他行不显示
                    if (firstMergedTransaction[stock_id] === trans_id) {
                        positionTitle = "合并仓位（同股票多笔交易的总仓位）";
                        positionColor = "#000"; // 黑色
                    } else {
                        positionText = ""; // 其他行不显示
                        positionColor = "transparent";
                    }
                } else {
                    positionColor = "#000";
                }
            ...
            $("#istock-table").children().children().append(`
				<tr class="istock-content" transactionid="${value.transaction_id}" stockid="${value.stock_id}">
					...
					<td class="istock-position mobile-hide" style="padding:5px;text-align:right;font-weight:bold;color:${positionColor};" title="${positionTitle}">${positionText}</td>
					...
					<td class="istock-merge mobile-hide" style="padding:4px;text-align:center">
						<input type="checkbox" class="istock-merge-checkbox" ${mergeSettings[value.transaction_id] ? 'checked' : ''}>
					</td>
					<td class="istock-exclude mobile-hide" style="padding:4px;text-align:center">
						<input type="checkbox" class="istock-exclude-checkbox" ${excludeSettings[value.transaction_id] ? 'checked' : ''}>
					</td>
				</tr>
			`);
        });
    ...
    function updateCashPosition() {
        const cash = (Number(userCash.company) || 0) + (Number(userCash.vault) || 0);
        let invested = 0;

        $("#istock-table tr.istock-content").each(function() {
            if ($(this).find(".istock-exclude-checkbox").is(":checked")) {
                return;
            }
            const value = $(this).find("td.istock-value").text().replace("$", "").replace(/,/g, "");
            invested += Number(value) || 0;
        });

        const total = cash + invested;
        const percentage = total > 0 ? (cash / total) * 100 : 0;
        $("#cash_position").text(`${percentage.toFixed(2)}%  ($${toThousands(Math.round(cash))})`);
    }
    function bindTableEvents() {
        // 单击图标快速定位股票
        $("td.istock-logo").off('click').on('click', function() {
            const stock_id = $(this).parent().attr("stockid");
            window.location.hash = "#" + stock_id;
            $("#" + stock_id).css("background-color", "lightgreen");
            setTimeout(() => { removeColor(stock_id) }, 1000);
        ...
        function checkAll(head_node, children_node) {
            head_node.css("cursor", "pointer");
            head_node.off('click').on('click', function() {
                const checkArray = children_node.toArray();
                let count = 0;
                $.each(checkArray, function(key, val) {
                    if (val.checked) {
                ...
                const newCheckedState = !(count == checkArray.length);

                // 更新所有复选框状态并保存到localStorage
                children_node.each(function() {
                    $(this).prop('checked', newCheckedState);
                    const row = $(this).closest('tr');
                    const transaction_id = row.attr('transactionid');

                    // 根据复选框类型更新对应的设置
                    let ls_object = getLocalStorage("ISTOCK", transaction_id) || {};

                    if ($(this).hasClass('istock-merge-checkbox')) {
                        ls_object.merge = newCheckedState ? 1 : 0;
                    } else if ($(this).hasClass('istock-exclude-checkbox')) {
                        ls_object.exclude = newCheckedState ? 1 : 0;
                    }

                    updateLocalStorage("ISTOCK", transaction_id, ls_object);
                });

                // 触发重新渲染（仅对合并和分红投资）
                if (children_node.hasClass('istock-merge-checkbox') || children_node.hasClass('istock-exclude-checkbox')) {
                    const sorted_res = getSortedTransactions();
                    renderTable(sorted_res);
                }
            });
        }
        ...
        // 监听复选框变化，重新渲染
        $(".istock-merge-checkbox, .istock-exclude-checkbox").off('change').on('change', function() {
            // 保存当前设置到localStorage
            const transaction_id = $(this).closest('tr').attr('transactionid');
            const merge_checked = $(this).closest('tr').find('.istock-merge-checkbox').is(':checked') ? 1 : 0;
            const exclude_checked = $(this).closest('tr').find('.istock-exclude-checkbox').is(':checked') ? 1 : 0;

            // 更新localStorage
            let ls_object = getLocalStorage("ISTOCK", transaction_id) || {};
            ls_object.merge = merge_checked;
            ls_object.exclude = exclude_checked;
            updateLocalStorage("ISTOCK", transaction_id, ls_object);
    ...
    function fetchUserMoney() {
        return new Promise((resolve, reject) => {
            const API = `https://api.torn.com/v2/user/money?key=${APIKey}`;
            console.log(`Request: ${API}`);
            fetch(API)
                .then((res) => {
                    if (res.ok) {
                    ...
                    reject('网络异常，无法获取现金数据');
                    return;
                }).then((res) => {
                    if (res && res.money) {
                        resolve({
                            company: res.money.company || 0,
                            vault: res.money.vault || 0
                        });
                    } else {
                        reject('响应数据格式错误');
                    }
    ...
    function updatePrice() {
        ...
        */
        ...
        // 首先收集所有标记为分红投资的交易ID（从投机仓位中剔除）
        let dividend_transactions = [];
        $("tr.istock-content").each(function() {
            const transcastion_id = $(this).attr("transactionid");
            const exclude_checked = $(this).children(".istock-exclude").children(":checked").length ? 1 : 0;
            if (exclude_checked) {
                dividend_transactions.push(transcastion_id);
            }
        });

        // 计算投机仓位总股本（排除分红投资）+ 现金（公司+金库）
        let speculative_cap = userCash.company + userCash.vault; // 初始值为现金总额
        $("tr.istock-content").each(function() {
            const transcastion_id = $(this).attr("transactionid");
            if (!dividend_transactions.includes(transcastion_id)) {
                const cap = $(this).children("td.istock-value").text().replace("$", "").replace(/,/g, "");
                speculative_cap += parseInt(cap);
            }
        });
        ...
        let stock_groups = {};
        $("tr.istock-content").each(function() {
            const stock_id = $(this).attr("stockid");
            const transcastion_id = $(this).attr("transactionid");
            const merge_checked = $(this).children(".istock-merge").children(":checked").length ? 1 : 0;
            const value = parseInt($(this).children("td.istock-value").text().replace("$", "").replace(/,/g, ""));

            if (!stock_groups[stock_id]) {
                stock_groups[stock_id] = { total_value: 0, transactions: [], merged_ids: [] };
            }
            stock_groups[stock_id].transactions.push(transcastion_id);

            // 如果标记为合并且不是分红投资，则累加价值
            if (merge_checked && !dividend_transactions.includes(transcastion_id)) {
                stock_groups[stock_id].total_value += value;
                stock_groups[stock_id].merged_ids.push(transcastion_id);
            }
        });
        ...
        target_nodes.each(function(index, value) {
            ...
            */
            ...
            const shares = parseInt($(this).siblings("td.istock-shares").text().replace(/,/g, ""));
            ...
            const profit = shares * price_change;
            ...
            // 计算并显示仓位百分比
            const is_dividend = dividend_transactions.includes(transcastion_id);
            let position_percent = 0;

            if (!is_dividend && speculative_cap > 0) {
                let transaction_value = parseInt($(this).siblings("td.istock-value").text().replace("$", "").replace(/,/g, ""));

                // 检查是否属于合并组
                if (stock_groups[stock_id] && stock_groups[stock_id].merged_ids.includes(transcastion_id)) {
                    transaction_value = stock_groups[stock_id].total_value;
                }

                // 计算仓位百分比
                position_percent = (transaction_value / speculative_cap) * 100;
            }

            // 显示仓位百分比
            if (is_dividend) {
                $(this).siblings("td.istock-position").text("分红").css("color", "#666");
            } else {
                $(this).siblings("td.istock-position").text(position_percent.toFixed(2) + "%");
            }
            ...
            */
            const merge = $(this).siblings(".istock-merge").children(":checked").length ? 1 : 0;
            const exclude = $(this).siblings(".istock-exclude").children(":checked").length ? 1 : 0;
